...
它更是一次很有意思的尝试：用**一个**端到端训练的模型，替掉**整套**多阶段架构——这背后是行业更大的趋势：**少一些人工设计的 pipeline 结构，多给模型一些自由，换取更高的天花板。**
...
- **生成式推荐只用在了召回上。** TIGER 这类方法把**召回**重新定义成生成 item ID，但生成之后，还是要接一整套下游的排序链路。

**OneRec 的思路：用一个生成式模型替掉整个多阶段架构。** 给定用户历史，**直接生成**最终的 session 视频列表——不再有独立的召回、粗排、精排模型。召回和排序，就是同一次自回归解码。
...
- **一个模型，没有多阶段架构的误差累积。** 把召回和排序统一起来，去掉了"每段都是下一段天花板"的瓶颈，也省掉了维护好几个独立训练模型的负担。更重要的是，OneRec 证明了这样一个统一的生成式推荐模型**能在工业级规模上真正上线，并且拿到实打实的收益**。
