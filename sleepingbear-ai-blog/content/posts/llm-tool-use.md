...
LLM 是冻结的文本预测器：知识是静态的，做算术不可靠，也看不到任何实时信息——今天的天气、你的数据库、某只股票的价格。直接问原始模型 *"巴黎现在天气怎么样？"*，它最多只能编出一个听起来合理的猜测。

**Tool Use** 弥补了这个鸿沟。但这座桥的搭法，是教会模型用一种外部软件（AI Agent）能识别并执行的格式去*请求*一个动作。模型产出一个函数调用请求，AI Agent 的 Runtime 去真正执行它。
